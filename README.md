...
A [Hail Query](https://hail.is/) based pipeline for QC'ing large-cohort germline genomic variant calling datasets.

1. Combines GVCFs (generated by GATK4) into a [VDS](https://hail.is/docs/0.2/vds/hail.vds.VariantDataset.html#hail.vds.VariantDataset).
2. Performs sample-level QC, including sex and ancestry inference.
3. Performs variant-level QC, including [allele-specific VQSR](https://gatk.broadinstitute.org/hc/en-us/articles/360035890551-Allele-specific-annotation-and-filtering-of-germline-short-variants).
